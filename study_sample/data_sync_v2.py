...
totalCount = mysql_cursor.fetchone()['count(1)']
print(totalCount)
pageSize = 50
pageNo = 428
...
try:
    while pageNo <= totalPage:
        ...
        log2disk(">>>>>>>totalCount:{0}  totalPage:{1}  pageSize:{2}  pageNo:{3}   startIdx:{4}  offset:{5}".format(totalCount, totalPage, pageSize, pageNo, startIdx, offset))
        ...
        print(ret);
        startId = ret[0]['id']
        endId = ret[len(ret)-1]['id']
        print(">>>>>>>处理从startId:{0} 到 endId:{1} 的数据 ... ".format(startId, endId))
        log2disk(">>>>>>>处理从startId:{0} 到 endId:{1} 的数据 ... ".format(startId, endId))
    ...
    #     print(results[0])
        endTime = time.clock()
        print("finish! endTime:{0} 耗时:{1}".format(endTime, (endTime-startTime)))
        log2disk("finish! endTime:{0} 耗时:{1}".format(endTime, (endTime-startTime)))
