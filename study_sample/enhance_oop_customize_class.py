...
'''
Traceback (most recent call last):
  File "enhance_oop_customize_class.py", line 154, in <module>
    print(v.speed)
...
# 错误信息很清楚告诉我们，没有找到speed这个attribute.
# 要避免这个错误，除了可以加上一个speed属性外，Python还有另外一种机制，那就是
# 写一个__getattr__()方法，动态返回一个属性。修改如下:
...
# 注意，只有在没有找到属性的情况下，才调用__getattr__, 已有属性,比如name, 不会在__getattr__中查找。
# 此外，注意到任意调用如v.abc都会返回None, 这是因为我们定义的__getattr__默认返回就是None。
# 要让class只相应特定的几个属性，我们就要按照约定，抛出AttributeError的错误:

class Vehicle4(object):

    def __getattr__(self, attr):
        if attr == 'age':
            return lambda: 23
        raise AttributeError('\'Vehicle\' object has no attribute \'%s\'' % attr)
...
那麽，怎么判断一个变量是对象还是函数呢? 其实，更多的时候，我们需要判断一个对象是否能被调用，
...
print(callable('str'))
'''
通过callablee()函数，我们就可以判断一个对象是否是"可调用"对象。

本节介绍的事最常用的几个定制方法，还有很多定制的方法，请参考Python的官方文档。
'''
