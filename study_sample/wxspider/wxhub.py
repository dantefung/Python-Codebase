...
def pipe_fakes(fake_name):
    ...
    while (True):
        rep = requests.get(
            Urls.query_biz.format(random=random.random(), token=Session.token, query=fake_name, begin=begin,
                                  count=count), cookies=Session.cookies, headers=Session.headers)
...
def pipe_articles(fakeid, query=''):
    ...
    if not artis.ret and page_limit:
        ...
        for i in range(0, len(mask)):
            ...
            rep = requests.get(
                Urls.query_arti.format(token=Session.token, fakeid=fakeid, begin=i * pagesize, count=pagesize,
                                       query=query), cookies=Session.cookies, headers=Session.headers)
            artis = ArtisResp(rep.text)
            if artis.ret:
                break
...
if __name__ == '__main__':
    ...
    parser.add_argument('-chrome', dest='chrome', type=str, help='可选:web chrome 路径, 默认使用环境变量中的，其次脚本同级目录下的chromedriver')
    ...
    parser.add_argument('-pipe', dest='pipe', type=str,
                        help='在method指定为pipe时, 该参数指定pipe处理流程. 例如:"pipe_example, pipe_example1, pipe_example2, pipe_example3"')
    parser.add_argument('-pl', dest='page_limit', type=str,
